...
from utils import load_csv_list, get_imgs_fn, crop_sub_imgs_fn, downsample_fn
...
def train():
    ...
    ###====================== PRE-LOAD DATA ===========================###
    train_hr_img_list = load_csv_list(path=config.TRAIN.img_list_path)
    ...
    train_hr_imgs = tl.vis.read_images(train_hr_img_list, n_threads=32)
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)) as sess:
        ...
        ###============================= LOAD VGG ===============================###
        vgg19_npy_path = "../pretrained_model/vgg19.npy"
        ...
        for epoch in range(n_epoch_init, n_epoch_init + 1):
            ...
            ## If your machine have enough memory, please pre-load the whole train set.
            for batch in tl.iterate.minibatches(inputs=train_hr_imgs, targets=train_hr_imgs, batch_size=batch_size, shuffle=True):
            #for idx in range(0, len(train_hr_imgs), batch_size):
                step_time = time.time()
                b_imgs_384 = tl.prepro.threading_data(batch[0], fn=crop_sub_imgs_fn, is_random=True)
...
def evaluate():
    ...
    # for im in train_hr_imgs:
    #     print(im.shape)
    valid_lr_imgs = tl.vis.read_images(valid_lr_img_list, n_threads=32)
    # for im in valid_lr_imgs:
    #     print(im.shape)
    valid_hr_imgs = tl.vis.read_images(valid_hr_img_list, n_threads=32)
    # for im in valid_hr_imgs:
    #     print(im.shape)
    # exit()
